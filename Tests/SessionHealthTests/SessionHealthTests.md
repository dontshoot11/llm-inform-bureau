...
| `NotificationTextTests` | What a notification says, on both sides of every phrase: the CLI command of its own service at the end, the mark in the title and the measurement in the body, a reset time it does not invent, no claim to have measured quality in either language, and the script the fallback channel goes out over — that a Russian title arrives whole, and that a quotation mark or a backslash in the question does not end the string early |
